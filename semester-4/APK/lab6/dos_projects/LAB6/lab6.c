...
/* IRQ8 protected-mode handler wrapper. */
void far rtc_handler(void);

/* IRQ1 protected-mode handler wrapper. */
void far keyboard_handler(void);
...
IDTPtr realmode_idt_ptr = { 0x03FF, 0x00000000UL };
...
/* Convert segment:offset into a 20-bit real-mode linear address. */
static u32 seg_off_to_linear(u16 seg, u16 off)
{
    return (((u32)seg) << 4) + (u32)off;
}
...
static void set_idt_entry(u16 vec, void (far *handler)(void), u16 selector, u8 type_attr)
{
...
void init_gdt(void)
{
    ...
    gdt_ptr.base  = seg_off_to_linear(rm_ds, (u16)&gdt[0]);
}
...
void init_idt(void)
{
    ...
    idt_ptr.base  = seg_off_to_linear(rm_ds, (u16)&idt[0]);
}
...
int main(void)
{
    ...
    /* Keep interrupts disabled while PIC is remapped and RTC is armed. */
    asm cli
