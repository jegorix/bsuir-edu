...
CODE_RM_BEGIN   = $                              ;начало сегмента кода реального режима
...
START:  
    ...
    lea dx,MSG_ENTER                             ;вывод сообщения
    mov ah,9h 
    int 21h 

    call INPUT                                   
    mov ds:[TIME], al                            ; кладем время в time
    
    call FILL_CR_0_BUFFER_RM                     ;сформировать буфер cr0
    lea dx, BUFFER_CR_0_RM                       ; вывести значение cr0 на экран
    mov ah, 9h 
    int 21h 

    lea dx,MSG_HELLO                              ; попросить ввести любой символ
    mov ah,9h 
    int 21h 

    mov ah,7h                                     ;считать символ без эха
    int 21h 


PREPARE_RTC:                                     ;Подготовка часов RTC 
    mov al,0Bh                                   
    out 70h,al                                   ;Выбрать регистр состояния 0Bh, в порт 70 кладем нужный регистр, в 71 - его значение
    in  al,71h                                   
    or  al,00000100b                             ;Установить бит DM в 1 - формат представления время в двоичном виде, сохранив предыдущую конфигурацию
    out 71h,al                                   ;Двоичный режим - для простоты подсчета, чтоб не в BSD
...
SAVE_MASK:                                       ;Сохранить маски прерываний, тк мы будем менять обработчики прерываний     
    in      al,21h 
    mov     INT_MASK_M,al                        ;master
    in      al,0A1h 
    mov     INT_MASK_S,al                        ;slave
...
LOAD_GDT:                                        ;Заполнить глобальную таблицу дескрипторов            
    mov ax,DATA                                  ;Формирование базового адреса в сегменте данных
    mov dl,ah                                     
    xor dh,dh 
    shl ax,4                                     ;формирование 20-разрядного адреса физической памяти(линейного адреса)
    shr dx,4 
    mov si,ax                                    ;si - младшая часть
    mov di,dx                                    ;di - старшая часть

WRITE_GDT:                                       ;Заполнить дескриптор GDT
    lea bx,GDT_GDT                               ;смещение дескриптора GDT
    mov ax,si 
    mov dx,di 
    add ax,offset GDT 
    adc dx,0                                    ;учесть перенос
...
;Процедура ввода время-нахождения в защищенном режиме ax - результат
INPUT proc near                                  
    mov ah,0ah 
    xor di,di 
    ...
    cmp byte ptr [si],"-"                      ;пропускаем минус
    jnz ii1 
    mov di,1  
    inc si    
II1: 
...
FILL_CR_0_BUFFER proc near      ; вывести значение cr0 на экран
	push eax 
	push esi 
	push dx 
	...
	mov eax, cr0 
	xor dx, dx 
	mov cx, 32              ; 32 итерации для вывода каждого бита регистра 
	lea esi, BUFFER_CR_0 
	fill_cr_0_loop: 
	mov dl, al  
	shl dl, 7           ; получение конкретного бита 
	shr dl, 7 
	shr eax, 1   ; смещаем 
	add dl, 48  ; '0'
	mov [esi], dl  ; кладем результат 
	inc esi  ; увеличиваем буфер 
	xor dl, dl 
	loop fill_cr_0_loop 
...
CLRSCR  proc near                                ;Процедура очистки консоли
    push es  
    pushad                                       ;запушить все регистры общего назначения
...
BUFFER_CLEAR  proc near                          ;Процедура очистки буфера, в esi кладем буфер
    mov al,' ' 
    mov [esi+0],al 
    mov [esi+1],al 
    mov [esi+2],al 
...
DATA_BEGIN      = $ 
    ...
    S_DESC  struc                                   ;Структура сегментного дескриптора
        LIMIT       dw 0                             ;0,1 - Лимит сегмента   
        BASE_L      dw 0                             ;2,3 - Адрес базы, младшая часть 
        BASE_M      db 0                             ;4 - Адрес базы, средняя часть
        ACCESS      db 0                             ;5 - Байт доступа
        ATTRIBS     db 0                             ;6 - Лимит сегмента и атрибуты(7: G(дробность) G=0 - в байтах, G=1 - в страницах; 6: D/B(разрядность) - =0: 16/8, =1: 32/8; 5: always 0; 4: avail; 3-0: лимит)
        BASE_H      db 0                             ;7 - Адрес базы, старшая часть
    S_DESC  ends         
    ...
    GDT_GDT     S_DESC <GDT_SIZE-1,,,10010010b,0,>  ; 7: p = 1 - присутствие в памяти; 5-6: DPL = 0 - уровень защиты(макс) ; 4: s = 1 - тип сегмента(сегмент кода или данных) ; 3:  сегмент данных; 2: e = 0 - направление расширения(вверх по адресам) ; 1: w = 1 - writeable(разрешактся запись(и чтение)) ; 0: a = 0 - востребованность сегмента(на магнитный диск)                
    GDT_CODE_RM S_DESC <SIZE_CODE_RM-1,,,10011010b,0,>  ; 7: p = 1 - присутствие в памяти; 5-6: DPL = 0 - уровень защиты(макс) ; 4: s = 1 - тип сегмента(сегмент кода или данных) ; 3:  сегмент программ; 2: с = 0 - confirming(разрешает обращение только когда CPL=DPL) ; 1: r = 1 - readable(разрешактся считывание) (редактировать нельзя, чтобы редактировать, нужно создать сегмент данных с тем же адресом с w = 1, и работать через него); 0: a = 0 - востребованность сегмента(на магнитный диск)                            
    GDT_DATA    S_DESC <SIZE_DATA-1,,,11110010b,0,>   ; 7: p = 1 - присутствие в памяти; 5-6: DPL = 11 - уровень защиты(мин) ; 4: s = 1 - тип сегмента(сегмент кода или данных) ; 3:  сегмент данных; 2: e = 0 - направление расширения(вверх по адресам) ; 1: w = 1 - writeable(разрешактся запись(и чтение)) ; 0: a = 0 - востребованность сегмента(на магнитный диск)             
    GDT_STACK   S_DESC <1000h-1,,,10010010b,0,>   ; 7: p = 1 - присутствие в памяти; 5-6: DPL = 0 - уровень защиты(макс) ; 4: s = 1 - тип сегмента(сегмент кода или данных) ; 3:  сегмент данных; 2: e = 0 - направление расширения(вверх по адресам) ; 1: w = 1 - writeable(разрешактся запись(и чтение)) ; 0: a = 0 - востребованность сегмента(на магнитный диск)                           
    GDT_TEXT    S_DESC <2000h-1,8000h,0Bh,11110010b,0,0>  
    GDT_CODE_PM S_DESC <SIZE_CODE_PM-1,,,10011010b,01000000b,>   ; 32/8 в байтах, ; 7: p = 1 - присутствие в памяти; 5-6: DPL = 0 - уровень защиты(макс) ; 4: s = 1 - тип сегмента(сегмент кода или данных) ; 3:  сегмент данных; 2: e = 0 - направление расширения(вверх по адресам) ; 1: w = 1 - writeable(разрешактся запись(и чтение)) ; 0: a = 0 - востребованность сегмента(на магнитный диск)                  
    GDT_IDT     S_DESC <SIZE_IDT-1,,,10010010b,0,>   ; 7: p = 1 - присутствие в памяти; 5-6: DPL = 0 - уровень защиты(макс) ; 4: s = 1 - тип сегмента(сегмент кода или данных) ; 3:  сегмент данных; 2: e = 0 - направление расширения(вверх по адресам) ; 1: w = 1 - writeable(разрешактся запись(и чтение)) ; 0: a = 0 - востребованность сегмента(на магнитный диск)                         
    ...
    BUFFER_CR_0			db 32 dup('?') 
    					db 1 dup(0)  
    BUFFER_CR_0_RM			db 32 dup('?'), 13, 10, "$"  	; буфер для вывода значения регистра cr0				    
